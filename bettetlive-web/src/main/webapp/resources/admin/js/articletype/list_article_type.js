	...
	createGrid : function(grId) {
		return $('#' + grId)
				.datagrid(
						{
							url : mainServer
									+ '/admin/specialarticletype/queryArticleTypeAllJson',
							rownumbers : true,
							singleSelect : false,
							pagination : true,
							autoRowHeight : true,
...
function toEditArticleType() {
	...
	var url = mainServer
			+ "/admin/specialarticletype/toEditArticleType?typeId=" + typeId;
	if (centerTabs.tabs('exists', '修改文章分类')) {
		centerTabs.tabs('select', '修改文章分类');
		var tab = centerTabs.tabs('getTab', '修改文章分类');
...
function toAddArticleType() {
	var centerTabs = parent.centerTabs;
	var url = mainServer + "/admin/specialarticletype/toAddArticleType";
	if (centerTabs.tabs('exists', '添加文章分类')) {
		centerTabs.tabs('select', '添加文章分类');
		var tab = centerTabs.tabs('getTab', '添加文章分类');
		var option = tab.panel('options');
		option.content = '<iframe class="page-iframe" src="'
				+ url
				+ '" frameborder="0" style="border:0;width:100%;height:100%;" scrolling="auto"></iframe>'
...
// 逻辑删除专题
function toDelArticleType(value) {
	$.messager.confirm("提示", "你确认此操作吗?", function(r) {
		if (r) {
			var typeId = boxDataGrid.datagrid("getSelected").typeId;
			$.ajax({
				url : mainServer + "/admin/specialarticletype/editTypeStatus",
				data : {
					"typeId" : typeId,
					"status" : value
				},
