...
var StoreGrid = {
	createGrid : function(grId) {
		return $('#' + grId).datagrid({
			url : mainServer + '/admin/newproduct/queryNewAllJson',
			rownumbers : true,
			singleSelect : false,
			pagination : true,
			autoRowHeight : true,
...
function toEditRecommend() {
	...
	$.ajax({
		type : "POST",
		url : mainServer + "/admin/newproduct/queryNewJson",
		data : {
...
function addRecommend() {
	...
	var productId = $("#productId").val();
	if (productId == '') {
		$.messager.alert('提示', "新品名称不能为空", 'info');
		return;
	}
...
function toBack() {
	window.location.href = mainServer + "/admin/newproduct/findList";
}