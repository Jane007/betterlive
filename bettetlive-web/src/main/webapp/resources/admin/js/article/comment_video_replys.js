...
function toBack() {
	window.location.href = mainServer + "/admin/specialcomment/findVideoReview";
}
...
function addComment(parentId, specialTypeChild) {
	...
	$.ajax({
		type : "POST",
		url : mainServer + "/admin/specialcomment/addVideoReplyComment",
		data : {
			'content' : replyMsg,
			'parentId' : parentId,
		...
		success : function(data) {
			...
			if ("succ" == obj.result) {
				...
				window.location.href = mainServer
						+ "/admin/specialcomment/findVideoIdList?id="
						+ commentId;
			} else {
...
function searchComment(parentId) {
	$.ajax({
		type : "POST",
		url : mainServer + "/admin/specialcomment/getVideoIsCustom",
		data : {
			'commentId' : parentId
		},
		async : false,
