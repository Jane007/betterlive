...
function isSubmit() {
	$('#specialform').form('submit',
	{
		...
		onSubmit : function() {
			...
			var specialType = $("#specialType").val(); // 专题类型
			...
			// 专题页面
			var specialPage = specialPage = document.specialform.file1.value
			if (specialPage == '' || specialPage == null) {
				$("#Page_msg").html("请选择专题页面");
				$("#specialPage").focus();
				return false;
			} else {
				$("#Page_msg").html("");
...
function toBack() {
	window.location.href = mainServer + "/admin/special/findList";
}