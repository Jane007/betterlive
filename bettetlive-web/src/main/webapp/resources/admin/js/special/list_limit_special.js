	...
	createGrid : function(grId) {
		return $('#' + grId)
				.datagrid(
						{
							url : mainServer
									+ '/admin/special/querySpecialAllJson?specialType=2',
							rownumbers : true,
							singleSelect : false,
							pagination : true,
							autoRowHeight : true,
...
function toEditSpecial() {
	...
	var url = mainServer + "/admin/special/toEditLimitSpecial?specialId="
			+ specialId + "&specialType=2&activityType=3";
...
function toAddSpecial() {
	var centerTabs = parent.centerTabs;
	var url = mainServer
			+ "/admin/special/toAddLimitSpecial?specialType=2&activityType=3";
...
function delLimitHomePic(status) {
	$.ajax({
