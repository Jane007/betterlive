...
$(function() {
	...
	$("#zyupload").zyUpload({
		...
		isadd : false, // 区分添加美食教程 true 还是添加美食教程类型 false
		/* 外部获得的回调接口 */
	...
	$('#specialform').form(
			'submit',
			{
				...
				onSubmit : function() {
					...
					var specialTitle = $("#specialTitle").val(); // 美食教程标题
					...
					var specialPage = $("#specialPage").val(); // 美食教程页面
					if (specialPage == "" || specialPage == null) {
						$("#Page_msg").html("请填写视频路径");
						$("#specialPage").focus();
						return false;
					} else {
						$("#Page_msg").html("");
					...
					var specialIntroduce = $("#specialIntroduce").val(); // 美食教程介绍
					...
					var dispatching = $("#dispatching").val();// 选择产品

					if (null == dispatching || dispatching == "") {

					} else {
						var speclist = $("#speclist").val();
					...
					var specialUrl2 = $("#specialCover").val(); // 美食教程封面
