...
$(function() {
	...
	$("#zyupload").zyUpload({
		...
		isadd : false, // 区分添加限量抢购 true 还是添加限量抢购类型 false
		/* 外部获得的回调接口 */
...
var StoreGrid = {
	createGrid : function(grId) {
		...
		{
			url : mainServer
					+ '/admin/product/queryProductAllJson?hasNoPro=1&isOnline=1&productStatus=1',
...
function isSubmit() {
	$('#specialform').form('submit',{
		url : mainServer + '/admin/special/addSpecial',
		onSubmit : function() {
			...
			var specialTitle = $("#specialTitle").val(); // 限量抢购标题
			...
			var specialIntroduce = $("#specialIntroduce").val(); // 限量抢购介绍
			...
			var specialUrl2 = $("#specialCover").val(); // 限量抢购封面
...
function toBack() {
	window.location.href = mainServer + "/admin/special/findLimitList";
}