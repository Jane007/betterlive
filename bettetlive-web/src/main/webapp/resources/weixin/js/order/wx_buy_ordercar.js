		...
		$(function(){
			//动态计算goodsBox高度
			var oheight = 0;
			var olenght = $('.orgoodslist-wrap').length;
			if(olenght <= 3){
				$('.residue span:first').html('共'+ olenght + '款商品')
			}else{
				$('.orgoodslist-wrap').each(function(index){
					if(index <= 2){
						oheight += $(this).height();
						$('.goodsBox').height(oheight)
					}
				});
				$('.residue span:first').html('剩余'+ (olenght -3) + '款商品');
				$('.unfold').removeClass('packup');
				//打开折叠面板
				$('.unfold').click(function(){
					if(!$(this).hasClass('packup')){
						$('.goodsBox').height('auto');
						$('.residue span:first').html('共'+ olenght + '款商品');
		...
		$('#integralSpanId').click(function(){
			if(goldDeductSpecIds == null || goldDeductSpecIds == "" || goldDeductSpecIds.length <= 0){
				return;
			}
			...
			if($(this).hasClass('on')){
				...
				if(clearMoney != null && clearMoney != "" && parseFloat(clearMoney) > 0){
					var addIntegral = (clearMoney/integralDeductionRate).toFixed(2);
					surplusIntegral = surplusIntegral + parseFloat(addIntegral);
				}
				goldNeedIntegral = 0;
	  ...
	  $(".inputArea2").keyup(function(e){
		...
		function totalCount(){
			...
			if($("#card3").is(':checked')){
				var singleCouponPrice=$("#singleCouponLineId em").text();      //红包金额
			...
			var totalPrice=parseFloat(orgPrice)+parseFloat(postagePrice)- parseFloat(activityPrice)-parseFloat(couponPrice)-parseFloat(singleCouponPrice)-parseFloat(fullCut);
			
			npayRealMoney = totalPrice;
			
			if(usecard=="useGiftCard" && checkGiftBox){
				//实付金额 = 总金额  - 礼品卡使用金额
				totalPrice=parseFloat(orgPrice)+parseFloat(postagePrice)- parseFloat(activityPrice)-parseFloat(fullCut);
			...
			if(goldDeductSpecIds != null && goldDeductSpecIds != "" 
					&& goldDeductSpecIds.length > 0 && integralCheckId && calculateMoney != null 
...
//				if(bool){
				...
				if(null !=ifCouponMoney && ""!=ifCouponMoney){
			//		var returnType='${returnType}';  //根据商品多少来判定是单个购买还是从购物车结算 
					window.location.href= mainServer + "/weixin/customercoupon/toChooseCouponInfo?returnType="+returnType+"&totalPrice="+ifCouponMoney;
				}else{
...
//				if(bool){
				//parseFloat($(".sumBox .price").text());  
				if(null !=ifCouponMoney && ""!=ifCouponMoney){
					window.location.href= mainServer + "/weixin/customercoupon/toChooseSingleCouponByProducts?returnType="+returnType+"&totalPrice="+ifCouponMoney+"&productIds="+ifCouponProIds+"&specIds="+ifCouponSpecIds;
				}else{
					showvaguealert('商品价格异常');
					return false;
				}
...
//					$(".gift-card").find(".giftcards").attr("lang","");
		...
		function submitForm(){
			...
			$('.goodsBox .calloff').each(function(i){
				if($(this).html() == "取消"){
					redeemSpecIds += $(this).attr("data-id") + ",";
				}
			})
			
			if(redeemSpecIds != null && redeemSpecIds != "" && redeemSpecIds.length > 0){
				redeemSpecIds = redeemSpecIds.substring(0, redeemSpecIds.length - 1);
			}
			
			var integralCheckId = $("#integralCheckId").attr('checked');
			var calculateMoney = $("#integralLineId").attr("data-calculate-money");
			var integralGoldSpecIds = "";
			if(integralCheckId && calculateMoney != null 
					&& calculateMoney != "" && parseFloat(calculateMoney) > 0){ //使用普通金币抵扣
				integralGoldSpecIds = goldDeductSpecIds+"";
			}
			...
			$.ajax({
				...
				data:{
					...
					"cartIds":cartIds,
					"goldDeductSpecIds":integralGoldSpecIds,
					"redeemSpecIds":redeemSpecIds,
					"token":token,
				...
				success:function(data){
					if(data.result=='succ'){
						//禁用去付款按钮，取消事件绑定
						$('.go-shop').addClass('disable').removeAttr('onclick');
						if(data.notify=='succ'){   //支付金额为0，直接跳转 ,不走微信支付
		...
		$(function(){
			...
			var url= mainServer + "/weixin/shoppingcart/toshoppingcar";
