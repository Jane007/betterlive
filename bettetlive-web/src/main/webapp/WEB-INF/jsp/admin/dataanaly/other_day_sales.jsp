...
<script type="text/javascript" src="${resourcepath}/admin/js/dataanaly/other_day_sales.js"></script>
<script type="text/javascript">
	...
	function queryMonthCanvas(){
		...
		$("#canvasForm").attr("action", "${mainserver}/admin/orderanaly/otherMonthSales");
	...
	function toOrders(queryDate){
		...
		var url ="${mainserver}/admin/otherorder/findList?queryDate="+queryDate;
...
<!-- 返回店铺年度表 -->
<form id="canvasForm" name="canvasForm" method="post" action="${mainserver}/admin/orderanaly/otherDaySales">
...
<div id="orderMonthSales" style="overflow:hidden;zoom:1;">
	...
	<input type="button" value="其他平台日营收报表" onclick="$('#canvasForm').submit();" style="float:left;">
...
<tr>
	<td>
		...
		<c:when test="${dayTotalMoneys == null || dayTotalMoneys.length() == 0}">
			<div align='center'>其他平台${queryDate}月份暂无营收数据</div>
		</c:when>
		<c:otherwise>
