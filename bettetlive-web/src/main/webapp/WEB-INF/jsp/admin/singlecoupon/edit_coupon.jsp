...
<html>
	...
	<body>
				...
				<div class="easyui-tabs1" style="width:100%;">
			      <div title="添加单品红包" data-options="closable:false" class="basic-info">
			      		...
			      		<form id="couponform" action=""  method="post" enctype="multipart/form-data">
					      	<input  type="hidden" name="couponId" id="couponId" value="${singleCouponVo.couponId }" style="height:35px;margin:0 0;width:280px;" maxlength="100"/>
					      	<table class="kv-table">
								<tbody>
									<tr>
										<td class="kv-label">单品红包名称&nbsp;<font color="red">*</font></td>
										<td class="kv-content">
											<input class="textbox" type="text" name="couponName" id="couponName" value="${singleCouponVo.couponName }" style="height:35px;margin:0 0;width:280px;" maxlength="100"/>
										...
										<td class="kv-content">
											满<input class="textbox" type="text" name="fullMoney" id="fullMoney" value="${singleCouponVo.fullMoney }"  style="height:35px;margin:0 0;width:80px;" maxlength="10" precision="0"/>
									...
									<tr>
										...
										<td class="kv-content">
											<input class="easyui-datetimebox" type="text" name="startTime" id="startTime" value="${singleCouponVo.startTime }"  style="height:35px;margin:0 0;width:220px;"/>
											<span id="Start_msg" style="color: red;"></span>
										</td>
										
										<td class="kv-label">红包结束时间&nbsp;<font color="red">*</font></td>
										<td class="kv-content">
											<input class="easyui-datetimebox" type="text" name="endTime" id="endTime" value="${singleCouponVo.endTime }"  style="height:35px;margin:0 0;width:220px;"/>
									...
									<tr>
										...
										<td class="kv-content" >
											<input class="numberbox" type="text" name="couponMoney" id="couponMoney" value="${singleCouponVo.couponMoney }" style="height:35px;margin:0 0;width:80px;"/>
											<span id="couponMoney_msg" style="color: red;"></span>
										</td>
										<td class="kv-label">是否推荐到首页&nbsp;</td>
										<td class="kv-content">
											<c:if test="${singleCouponVo.homeFlag != null && singleCouponVo.homeFlag == 1}">
												<input type="radio" name="homeFlag" value="1" checked="checked"/>是
												<input type="radio" name="homeFlag" value="0"/>否
											</c:if>
											<c:if test="${singleCouponVo.homeFlag != null && singleCouponVo.homeFlag == 0}">
												<input type="radio" name="homeFlag" value="1"/>是
												<input type="radio" name="homeFlag" value="0" checked="checked"/>否
											</c:if>
										</td>
									</tr>
									<tr>
										...
										<td class="kv-content" >
											<input class="textbox" type="text" name="limitCopy" id="limitCopy" style="height:35px;margin:0 0;width:80px;" value="${singleCouponVo.limitCopy}" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)"/>
												<span id="limitCopy_msg" style="color: red;">-1代表不限份数</span>
										</td>
										<td class="kv-label">跳转页面&nbsp;</td>
										<td class="kv-content">
											<input type="radio" name="jumpToPage" id="jumpToPage" value="1" <c:if test="${singleCouponVo.jumpToPage != null && singleCouponVo.jumpToPage == 1}">checked="checked" </c:if>/>跳转商品详情
											<input type="radio" name="jumpToPage" id="jumpToPage" value="2" <c:if test="${singleCouponVo.jumpToPage != null && singleCouponVo.jumpToPage == 2}">checked="checked" </c:if>/>跳转购买页面										
										</td>
									</tr>
									<tr>
										<td class="kv-label">红包说明&nbsp;</td>
										<td class="kv-content" colspan="3">
											<input class="easyui-textbox" type="text" name="couponContent" value="${singleCouponVo.couponContent }" id="couponContent" style="height:35px;margin:0 0;width:90%;"/>
							...
							<table class="kv-table">
								<tbody>
									<tr>
										...
										<td class="kv-content" colspan="3">
										<input  type="hidden" name="couponBanner" id="couponBanner" value="${singleCouponVo.couponBanner}"/>
										<span id="couponBanner1" style="color: red;"></span>
											<div id="zyupload">
											</div>
										</td>
										<td class="kv-label" colspan="2" id="pics">
										<img id="pimg" src="${singleCouponVo.couponBanner}" width="160px"/>
										</td>
									</tr>
								</tbody>
							</table>
		...
		<script type="text/javascript" src="${resourcepath}/admin/js/singlecoupon/edit_coupon.js"></script>
	</body>
</html>