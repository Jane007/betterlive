...
<html>
	...
	<body>
		  <table id="StoreGrid" style="width:100%;height:529px;" title="限时抢购列表">
	        
	      </table>
	      
	      <div id="storeToolbar" style="padding:0 30px;">
	      		<div id="tb" style="padding:0 30px;">
	      		       <input type="hidden" name="specialType" id = "specialType" value="2">
				             抢购名称: <input class="easyui-textbox" type="text" name="specialName"  id="specialName" maxlength="30" style="width:166px;height:35px;line-height:35px;" />
...
<!-- 				       <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" data-options="iconCls:'icon-list'" id="homeCover"   onclick="addOpenDlg()">首页封面</a> -->
		...
		<div id="dlg" class="easyui-dialog" title="编辑封面图" data-options="closed:true"  fit=“true”  style="width:520px;height:320px;padding:10px;">
    		<form  id="form1" action="" method="post"  enctype="multipart/form-data">
    			<table class="kv-table">
					<tbody>
						<tr>
							<td class="kv-label"> 限量抢购首页封面图:&nbsp;<font color="red">*</font></td>
							<td class="kv-content">
							...
							<td colspan="2">
								<div align="center" style="margin-top: 60px;">
									<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" data-options="selected:true" 
									   onclick="editLimitHomePic()" id="saveBtn" >保存</a>
								   	<c:if test="${limitPic != null && limitPic.picture_id > 0}">
							   			<c:if test="${limitPic.status == 0}">
										  	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" data-options="selected:true" 
										   		onclick="delLimitHomePic(1)" id="upBtn" >上架</a>
										</c:if>
				<%-- 					
									  <c:if test="${limitPic.status == 1}">
									  	<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" data-options="selected:true" 
									   		onclick="delLimitHomePic(0)" id="downBtn" >下架</a>
									   </c:if>
			   --%>
									</c:if>
									...
									<input type="hidden" name="picture_id" id="picture_id" value="${limitPic.picture_id}"/>
									<input type="hidden" name="picture_type" id="picture_type" value="${limitPic.picture_type}"/>
									<input type="hidden" name="original_img" id="original_img" value="${limitPic.original_img}"/>
	    ...
	    <script type="text/javascript" src="${resourcepath}/admin/js/special/list_limit_special.js"></script>
	</body>
</html>