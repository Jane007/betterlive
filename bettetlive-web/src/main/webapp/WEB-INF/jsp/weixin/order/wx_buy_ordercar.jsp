...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			var goldDeductSpecIds = '${goldDeductSpecIds}';
	...
	<body>
		...
		<div class="container">
				...
				<div class="goodsBox-wrap">
					...
					<c:if test="${listProduct.size() > 3}">
					<div class="residue">
						<span>剩余${listProduct.size() - 3}款商品</span><span class="unfold packup"></span>
					</div>
					</c:if>
				</div>
				<div class="compile editcom">
					<ul>
						...
						<li class="gift-card <c:if test='${userMoney==null  }'>empty</c:if>">
						...
						<c:if test="${fn:length(goldDeductSpecIds) > 0}">
						<li id="integralLineId" class="redact" style="border-bottom:0;display: none;" data-redeem-spec-ids="${goldDeductSpecIds}" data-calculate-money="0">
				...
				<div class="sumBox">
					<ul>
						...
						<li>
							<label for="">运费</label>
							<c:if test="${freePostage ==0}">
								<span><i class="freight">免运费</i></span>
							</c:if>
							<c:if test="${freePostage >0}">
								<span>￥<i class="freight" >${postage}</i></span>
							</c:if>
							<c:if test="${freePostage >0}">
								<em>再买${freePostage}元零食类商品免邮</em>
							</c:if>
						</li>
						...
						<c:if test="${fullCut != null && fullCut > 0}">
							<li>
...
<%-- 									<em>${salePromotion.promotionName}</em> --%>
	...
	<script type="text/javascript" src="${resourcepath}/weixin/js/common.js"></script>
	<script type="text/javascript" src="${resourcepath}/plugin/jquery.md5.js"></script>
	<script type="text/javascript" src="${resourcepath}/weixin/js/order/wx_buy_ordercar.js?t=201804221531"></script>
