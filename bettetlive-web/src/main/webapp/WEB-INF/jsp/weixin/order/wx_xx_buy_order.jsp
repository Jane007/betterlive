...
<html>
	<head>
	    ...
	    <link rel="stylesheet" href="${resourcepath}/weixin/css/editOrder.css" />
		...
		<div class="container">
			...
			<div class="mainBox">
				...
				<div class="goodsBox">
					<c:forEach items="${listProduct}" var="l" varStatus="status">
						<div class="orgoodslist">
							...
							<div class="orgoodsright">
								<p class="orgoodsnum">x${l.amount}</p>
							</div>
				...
				<div class="payType">
					...
					<em>
						<i>微信支付</i>
						<span class="on"><input type="radio" checked="checked" name="pay" value="1"/></span>
					</em>
				</div>
				<div class="consent">
					<span class="on"><input type="checkbox" checked="checked"/></span>
					<em>我已同意<a>《挥货服务协议》</a></em>
				</div>
			</div>
			<div class="botShop">
				<span class="newBuy">实付：¥ <em></em></span>
				<span class="go-shop" onclick="submitForm()">去付款</span>
	...
	<script type="text/javascript">
		...
		var zhekou='${zhekou}';   //活动优惠价格 
		var listCustomerCoupon='${listCustomerCoupon}';    //优惠券是否为空  
		var couponInfo='${couponInfo}';  //用户选择的优惠券		
		...
		$(function(){
			...
			function totalCount(){
				var productPrice=$(".sumBox .price").text();  
				
				 //商品总金额 + 运费  -活动优惠价格 -优惠券金额  -礼品卡使用金额
		...
		function submitForm(){
			...
			$.ajax({
				url:'${mainserver}/weixin/payment/payxx',
				data:{
				...
				success:function(data){
					if(data.result=='succ'){
						...
						if(data.notify=='succ'){   //支付金额为0，直接跳转 
							var tradeNo=data.orderCode;
							window.location.replace("${mainserver}/weixin/order/queryOrderDetails?orderCode="+tradeNo);  //从支付完成进入订单详情 
						}else{
							var record = data;
		   ...
		   WeixinJSBridge.invoke(
		       ...
		       function(res){ 
		    	   ...
		    	   if(res.err_msg == "get_brand_wcpay_request:ok") {
		    		   window.location.replace("${mainserver}/weixin/order/queryOrderDetails?orderCode="+orderCode);  //从支付完成进入订单详情
		           } else if (res.err_msg == "get_brand_wcpay_request:cancel"){
		        	   showvaguealert('您已取消支付');
		        	   setTimeout(function(){
		        		   //订单去掉支付，进入订单倒计时页面
		        		   window.location.replace("${mainserver}/weixin/order/toPayOrderDetail?orderId="+orderId+"&orderCode="+orderCode);
		        	   },500);
		           } else {
		    		   showvaguealert('出错了');
		           } 
	...
	<script type="text/javascript">
		$(function(){
			...
			if(1==returnType){
				...
				url="${mainserver}/weixin/product/toxxgoodsdetails?productId=${productId}";
			}
