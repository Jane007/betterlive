...
<html>
	<head>
		...
		<link rel="stylesheet" href="${resourcepath}/weixin/css/myAddress.css?t=201801041416" />
	...
	<body>
		...
		<div class="container">
			<div class="mainBox" style="top: 0rem;">
				<c:if test="${listReceiverAddress != null && fn:length(listReceiverAddress)>0 }">
					<div class="consigneebox">
						<c:forEach items="${listReceiverAddress}"  var="ra">
							...
							<c:choose>
								<c:when test="${ null !=receiverId && ''!=receiverId && receiverId==ra.receiverId}">
									<div class="addresslist active"  id="${ra.receiverId}">
								</c:when>
								<c:otherwise>
								...
								<div class="add-top">
									...
									<p class="defaultaddress">
										...
										<a href="${mainserver}/weixin/addressmanager/toAddReceiverAddressFromOrder?returnType=${returnType}&receiverId=${ra.receiverId}" class="addressedit">编辑</a>
									</p>
								</div>
							</div>
						</c:forEach>	
				...
				<a href="${mainserver}/weixin/addressmanager/toAddReceiverFromOrder?returnType=${returnType}&receiverId=${receiverId}" class="newAdd-address">
					新增地址
				</a>
			</div>
		</div>
	...
	<script src="${resourcepath}/weixin/js/receiveraddress/wx_choose_orderreceiver.js"></script>
</html>