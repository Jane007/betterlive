...
<body>
		...
		<div class="mainBox" style="top:0rem;">
			...
			<div class="goodsContent">
				<div class="goodsInfor protitlebox">
					 ...
					 <div class="projiage">
					 	<p><span>挥货价</span>
				 			<c:choose><c:when test="${productInfo.listSpecVo[0] != null && productInfo.listSpecVo[0].activity_price != null 
												&& productInfo.listSpecVo[0].activity_price != '' && productInfo.listSpecVo[0].activity_price != '-1'}">
									<span>￥${productInfo.listSpecVo[0].activity_price}</span><strong>￥${productInfo.listSpecVo[0].spec_price}</strong>
								</c:when>
								<c:when test="${productInfo.listSpecVo[0] != null && productInfo.listSpecVo[0].discount_price != null 
									...
									<span>￥${productInfo.listSpecVo[0].discount_price}</span><strong>￥${productInfo.listSpecVo[0].spec_price}</strong>
								</c:when>
								<c:otherwise>
				...
				<div class="parameterBox newprocsbox">
					<ul class="guige">
						...
						<c:if test="${specialVo != null && specialVo.specialId > 0 && specialVo.specialType == 1}">
							<li onclick="location.href='${specialVo.specialPage}'">
				...
				<c:if test="${comments.size() > 0}">
					<div class="evaluateBox plbox">
						 <div class="pltitle" onclick="location.href='${mainserver}/weixin/productcomment/findList?type=1&productId=${productInfo.product_id}'">
				...
				<c:forEach items="${productInfo.listSpecVo}"  var="s">
					<c:choose>
						...
						<c:when test="${s.discount_price !=null && s.discount_price >= 0}">
							<li onclick='getPromotion(${s.spec_id});' value="¥${s.discount_price}" data-orgPrice="¥${s.spec_price}" id="${s.spec_id}" data-copy="${s.stock_copy }" 
							data-limit="${s.limit_max_copy }" data-hasBuy="${s.hasBuy_copy }" data-restCopy="${s.rest_copy}"
							data-carNums="${s.carNums }" data-carCanAdd="${s.carCanAdd }" data-package="${s.package_desc }"
							data-promoitionName="${s.promoitionName }" data-fullMoney="${s.fullMoney }" data-cutMoney="${s.cutMoney }" data-single="${s.singleCouponCount}" data-coupon="${s.couponCount}"
							>${s.spec_name}
	...
	var link = '${mainserver}/weixin/product/towxgoodsdetails?productId=${productInfo.product_id}&specialId=${specialVo.specialId}&source=${orderSource}';
	var back = link;
	var imgUrl = "${productInfo.product_logo}";
	...
	<script src="${resourcepath}/weixin/js/goods/wx_goodsdetails.js?t=201802261619"></script>
