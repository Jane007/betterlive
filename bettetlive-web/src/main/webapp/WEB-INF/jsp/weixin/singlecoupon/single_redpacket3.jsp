...
<html>
	<head>
    	...
    	<link rel="stylesheet" href="${resourcepath}/weixin/css/common2.css?v=201709081724" />
  		<link rel="stylesheet" href="${resourcepath}/weixin/css/new-getRedpacket.css?v=201709081724" />
	...
	<body class="dpbgbox">
		...
		<div class="dptitlebox">
			<span>${singleCouponVo.couponName}</span>
...
<!-- 				<div id="couponShow" class="oldUser tyyiling tydailing"  style="display: none;">  -->
	...
	var jumpToPage = '${singleCouponVo.jumpToPage}';
	$(function(){
			...
			$('.now-get').click(function(){
				...
				if(!!window.TuiaAdverter) { //先判断js是否引入成功       
					TuiaAdverter.init(function() {
						checkReg(sMobile, code, couponId, specId);
					}, 
					{
						"location": "register"
					}) 	
	...
	function checkReg(sMobile, code, couponId, specId){
		$.ajax({
			type: 'POST',
	...
	function subPay(){
		...
		var ordSource = $("#orderSource").val();
		if(ordSource != null && ordSource != ""){
		 ...
		 if(!!window.TuiaAdverter) { //先判断js是否引入成功       
			TuiaAdverter.init(function() {
				if(jumpToPage!=null&&jumpToPage==1){
					$(".now-old").html("商品详情");
					window.location.href = "${mainserver}/weixin/product/towxgoodsdetails?productId="+productId+ordSource+"&specId="+specId;
				}else{
					$("#buyForm").submit();
				}
			}, 
