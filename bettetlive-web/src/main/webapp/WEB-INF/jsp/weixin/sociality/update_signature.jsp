				...
				<div class="contentBox">
					<div class="identityBox">
						<form action="" class="verification" id="phoneForm">
							<ul> 
								<li class="fbtitle">  
									<input type="text" placeholder="请输入个性签名,限25个字以内" value="${signature}" name="signature" id="signature" onchange="fontxian()"   maxlength="25"/> 
									<span class="fontxian">25</span>
								</li>
							</ul>   
							<div class="nextStep" id="msgNext" onclick="javascript:submitSignature();">
							       保存
						 	</div>
						</form>					 
					</div>
 		...
 		if(fontXf>=0){
			...
			function submitSignature(){
				...
				if(signature == null || signature == "" || signature.length <= 0){
					showvaguealert("签名不能为空");
					return;
				}
				if(signature.length > 25){
					showvaguealert("限制25个字");
					return;
				}
				$.ajax({ 
			        type: "POST", 
			        ...
			        dataType: "JSON", 
			        async: false, 
			        url: "${mainserver}/weixin/customer/editSignature",
