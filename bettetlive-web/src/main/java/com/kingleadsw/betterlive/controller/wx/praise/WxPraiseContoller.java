package com.kingleadsw.betterlive.controller.wx.praise;
...
@Controller
@RequestMapping(value = "/weixin/praise")
public class WxPraiseContoller extends AbstractWebController{
	private static Logger logger = Logger.getLogger(WxPraiseContoller.class);
	@Autowired
...
//						subMsgType = 19;
			...
			logger.error("/weixin/praise/addPraise----error:"+ e);
			return CallBackConstant.FAILED.callback();
		}
	}
	...
	public Map<String, Object> canselPraise(HttpServletRequest request,
		...
		try {
			...
			int count = praiseManager.deleteByPrimaryKey(pd.getInteger("praiseId"));
...
//				else if(praiseVo.getPraiseType() == 2){	//视频点赞
//					subMsgType = 21;
//				}
...
//						subMsgType = 19;
					...
					SpecialCommentVo spcmmtVo = specialCommentManager.queryCommentVideoById(praiseVo.getObjId());
					SpecialArticleVo sav = this.specialArticleManager.selectByPrimaryKey(spcmmtVo.getSpecialId());
					if(sav.getArticleFrom() == 0){ //精选
						subMsgType = 18;
					}else{	//动态
			...
			logger.error("/weixin/praise/cancelPraise----error:"+ e);
			return CallBackConstant.FAILED.callback();
		}
	}
}
